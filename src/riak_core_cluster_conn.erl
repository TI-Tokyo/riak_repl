...
init([Remote, test]) ->
    ?LOG_DEBUG("connecting to ~p", [Remote]),
...
connecting(timeout, State=#state{remote=Remote}) ->
    ...
    %% manager keep trying.
    ?LOG_DEBUG("cluster_conn: client timed out waiting for ~p", [Remote]),
    {stop, giving_up, State};
connecting({connect_failed, Error}, State=#state{remote=Remote}) ->
    ?LOG_DEBUG("ClusterManager Client: connect_failed to ~p because ~p."
                    " Will retry.",
                    [Remote, Error]),
    ?LOG_WARNING("ClusterManager Client: connect_failed to ~p because ~p."
                      " Will retry.",
                      [Remote, Error]),
...
connecting({connected_to_remote, Socket, Transport, Addr, Props, ProtoVersion}, State) ->
    ...
    ?LOG_DEBUG("Cluster Manager control channel client connected to"
...
connecting(Other, State=#state{remote=Remote}) ->
    ?LOG_ERROR("cluster_conn: client got unexpected "
                    "msg from remote: ~p, ~p",
                    [Remote, Other]),
    {next_state, connecting, State}.
...
handle_info({TransClosed, Socket} = Msg,
    ...
    ?LOG_DEBUG("Stopping because it looks like the connect closed: ~p", [Msg]),
    {stop, connection_closed, State};
handle_info({_ClusterManager, poll_cluster}, StateName, State) ->
    gen_fsm:send_event(self(), poll_cluster),
    {next_state, StateName, State};
handle_info(Msg, StateName, State) ->
    ?LOG_WARNING("Unmatch message ~p", [Msg]),
    {next_state, StateName, State}.

terminate(Reason, StateName, _State) ->
    ?LOG_DEBUG("Exiting while in state ~s due to ~p", [StateName, Reason]),
    ok.
